...
import Obb2Tree from '/experiments_old/collision/obb2/Obb2Tree';
...
function measure(separated) {
  if (separated && nonEmpty(separated[0]) && nonEmpty(separated[1])) {
    var dir = separated[2];
    var node1 = Obb2Tree.wrap(separated[0], dir);
    var node2 = Obb2Tree.wrap(separated[1], dir);
    var score = -Obb2.penetration(node1.box, node2.box);
    return {
      score: score,
      node1: node1,
      node2: node2,
...
function fromList(list) {
  ...
  var allLeaves = list.map(function(item) {
    var res = new Obb2Tree();
    Obb2.set(res.box, item);
    res.process();
    return res;
  });
